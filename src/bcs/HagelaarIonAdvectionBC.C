...
HagelaarIonAdvectionBC::computeQpResidual()
{
	...
	return _test[_i][_qp] * (1. - _r) / (1. + _r) * ((2 * _a - 1) * _sgn[_qp] * _mu[_qp] * -_grad_potential[_qp] * std::exp(_u[_qp]) * _normals[_qp]);
}
...
HagelaarIonAdvectionBC::computeQpJacobian()
{
	...
	return _test[_i][_qp] * (1. - _r) / (1. + _r) * ((2 * _a - 1) * _sgn[_qp] * _mu[_qp] * -_grad_potential[_qp] * std::exp(_u[_qp]) * _phi[_j][_qp] * _normals[_qp]);
}
