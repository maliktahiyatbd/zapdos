...
SecondaryElectronBC::computeQpOffDiagJacobian(unsigned int jvar)
{
	...
	if (jvar == _potential_id)
	{
		...
		return - _test[_i][_qp]	* 2. / (1. + _r) * (1. - _a) * _se_coeff[_qp] * _d_ion_flux_d_potential * _normals[_qp];
	}
	...
	else if (jvar == _ip_id)
	{
		...
		return -_test[_i][_qp]	* 2. / (1. + _r) * (1. - _a) * _se_coeff[_qp] * _d_ion_flux_d_ip * _normals[_qp];
	}
