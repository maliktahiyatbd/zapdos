...
//	_v_thermal = (_r_units / _t_units) * std::sqrt(8.0 * _e[_qp] * 2.0 / 3.0 * _actual_mean_en / (M_PI * _massem[_qp]));
	...
	_emission_flux = SchottkyEmissionBC::emission_flux() ;
...
//	_v_thermal = (_r_units / _t_units) * std::sqrt(8.0 * _e[_qp] * 2.0 / 3.0 * _actual_mean_en / (M_PI * _massem[_qp]));
	...
	if (jvar == _potential_id) {
		...
		_d_emission_flux_d_u = SchottkyEmissionBC::d_emission_flux_d_em() ;
	} else if (jvar == _ip_id) {
		_d_emission_flux_d_u = SchottkyEmissionBC::d_emission_flux_d_ip() ;
	}
