...
HagelaarEnergyBC::computeQpJacobian()
{
  ...
  // std::numeric_limits<double>::epsilon()) * -_grad_potential[_qp] * _normals[_qp] *
  ...
  /*	return	-(2 * _a - 1) * _d_trans_mean_en_d_u * ((1 - _r) / (1 + _r)) * _mumean_en[_qp] *
     -_grad_potential[_qp] * _normals[_qp] +
        -(2 * _a - 1) *   _trans_mean_en     * ((1 - _r) / (1 + _r)) *
     _d_mumean_en_d_actual_mean_en[_qp] * _actual_mean_en * _phi[_j][_qp] * -_grad_potential[_qp] *
     _normals[_qp] +
            (5/6) * _d_trans_mean_en_d_u * ((1 - _r) / (1 + _r)) * _v_thermal +
            (5/6) *   _trans_mean_en     * ((1 - _r) / (1 + _r)) * _d_v_thermal_d_u ;
  */
  return _test[_i][_qp] / (6. * (_r + 1.)) * (_r - 1.) *
         ((std::exp(_u[_qp]) * _phi[_j][_qp] - _se_energy[_qp] * _d_n_gamma_d_u) *
              (6. * -_grad_potential[_qp] * _normals[_qp] * _mumean_en[_qp] * (2. * _a - 1.) -
               5. * _v_thermal) +
          (std::exp(_u[_qp]) - _se_energy[_qp] * _n_gamma) *
              (6. * -_grad_potential[_qp] * _normals[_qp] * _d_mumean_en_d_actual_mean_en[_qp] *
                   _actual_mean_en * _phi[_j][_qp] * (2. * _a - 1.) -
               5. * _d_v_thermal_d_u));
}
...
HagelaarEnergyBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  _v_thermal = (_r_units / _t_units) *
               std::sqrt(8 * _e[_qp] * 2.0 / 3 * _actual_mean_en / (M_PI * _massem[_qp]));
  ...
  if (jvar == _potential_id)
  {
    ...
    // _normals[_qp] / (-_grad_potential[_qp] * _normals[_qp] +
    // std::numeric_limits<double>::epsilon()) - _ion_flux * _normals[_qp] /
    // (std::pow(-_grad_potential[_qp] * _normals[_qp], 2.) +
    // std::numeric_limits<double>::epsilon()) * -_grad_phi[_j][_qp] * _normals[_qp]);

    return _test[_i][_qp] / (6. * (_r + 1.)) *
           (10. * _d_ion_flux_d_potential * _normals[_qp] * _se_energy[_qp] * _se_coeff[_qp] *
                (_a - 1.) * (_r + 1.) +
            (_r - 1.) * ((-_se_energy[_qp] * _d_n_gamma_d_potential) *
                             (6. * -_grad_potential[_qp] * _normals[_qp] * _mumean_en[_qp] *
                                  (2. * _a - 1.) -
                              5. * _v_thermal) +
                         (std::exp(_u[_qp]) - _se_energy[_qp] * _n_gamma) *
                             (6. * -_grad_phi[_j][_qp] * _normals[_qp] * _mumean_en[_qp] *
                              (2. * _a - 1.))));
  }
  else if (jvar == _em_id)
  {
