...
NeumannCircuitVoltageMoles_KV::computeQpResidual()
{
	...
	_v_e_th = std::sqrt(8 * _e[_qp] * 2.0 / 3 * std::exp(_mean_en[_qp] - _em[_qp]) / (M_PI * _massem[_qp]));
	_v_i_th = std::sqrt(8 * _kb[_qp] * _T_heavy[_qp] / (M_PI * _mass[_qp]));

	return _test[_i][_qp] * _eps[_qp] * (-2. * (1. + _r) * _u[_qp] - 2. * (1. + _r) * _V_bat.value(_t, _q_point[_qp]) + _data.electrode_area() * _e[_qp] * _data.ballast_resist() / _voltage_scaling * (-1. + _r) * ((-1. + (-1. + _a) * _se_coeff[_qp]) * _N_A[_qp] * std::exp(_ip[_qp]) * _v_i_th + _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) * _v_e_th)) / (2. * _data.electrode_area() * _e[_qp] * ((-1. + 2. * _a) * (_muem[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) - (-1. + 2. * _b) * (-1. + (-1. + _a) * _se_coeff[_qp]) * (_muip[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * std::exp(_ip[_qp])) * _data.ballast_resist() * (-1. + _r));
}
...
NeumannCircuitVoltageMoles_KV::computeQpJacobian()
{
	...
	_numerator = (-2. * (1. + _r) * _u[_qp] - 2. * (1. + _r) * _V_bat.value(_t, _q_point[_qp]) + _data.electrode_area() * _e[_qp] * _data.ballast_resist() / _voltage_scaling * (-1. + _r) * ((-1. + (-1. + _a) * _se_coeff[_qp]) * _N_A[_qp] * std::exp(_ip[_qp]) * _v_i_th + _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) * _v_e_th));
	_denominator = (2. * _data.electrode_area() * _e[_qp] * ((-1. + 2. * _a) * (_muem[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) - (-1. + 2. * _b) * (-1. + (-1. + _a) * _se_coeff[_qp]) * (_muip[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * std::exp(_ip[_qp])) * _data.ballast_resist() * (-1. + _r));

	_d_denominator_d_u = 2. * _data.ballast_resist() * _e[_qp] * _data.electrode_area() * (-1. + _r) * (-1. + 2. * _a) * (_muem[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * -_d_n_gamma_d_u;
...
NeumannCircuitVoltageMoles_KV::computeQpOffDiagJacobian(unsigned int jvar)
{
	if (jvar == _ip_id)
	{
		...
		_d_numerator_d_ip = _data.electrode_area() * _e[_qp] * _data.ballast_resist() / _voltage_scaling * (-1. + _r) * ((-1. + (-1. + _a) * _se_coeff[_qp]) * _N_A[_qp] * std::exp(_ip[_qp]) * _phi[_j][_qp] * _v_i_th + _N_A[_qp] * (-_d_n_gamma_d_ip) * _v_e_th);
		_d_denominator_d_ip = (2. * _data.electrode_area() * _e[_qp] * ((-1. + 2. * _a) * (_muem[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * (-_d_n_gamma_d_ip) - (-1. + 2. * _b) * (-1. + (-1. + _a) * _se_coeff[_qp]) * (_muip[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * std::exp(_ip[_qp]) * _phi[_j][_qp]) * _data.ballast_resist() * (-1. + _r));
	...
	else if (jvar == _em_id)
	{
		...
		_v_e_th = std::sqrt(8 * _e[_qp] * 2.0 / 3 * std::exp(_mean_en[_qp] - _em[_qp]) / (M_PI * _massem[_qp]));
		_d_v_e_th_d_em = 0.5 / _v_e_th * 8 * _e[_qp] * 2.0 / 3 * _actual_mean_en / (M_PI * _massem[_qp]) * -_phi[_j][_qp];
		_v_i_th = std::sqrt(8 * _kb[_qp] * _T_heavy[_qp] / (M_PI * _mass[_qp]));

		_numerator = (-2. * (1. + _r) * _u[_qp] - 2. * (1. + _r) * _V_bat.value(_t, _q_point[_qp]) + _data.electrode_area() * _e[_qp] * _data.ballast_resist() / _voltage_scaling * (-1. + _r) * ((-1. + (-1. + _a) * _se_coeff[_qp]) * _N_A[_qp] * std::exp(_ip[_qp]) * _v_i_th + _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) * _v_e_th));
		_denominator = (2. * _data.electrode_area() * _e[_qp] * ((-1. + 2. * _a) * (_muem[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) - (-1. + 2. * _b) * (-1. + (-1. + _a) * _se_coeff[_qp]) * (_muip[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * std::exp(_ip[_qp])) * _data.ballast_resist() * (-1. + _r));

		_d_numerator_d_em = _data.electrode_area() * _e[_qp] * _data.ballast_resist() / _voltage_scaling * (-1. + _r) * (_N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) * _d_v_e_th_d_em + _N_A[_qp] * (std::exp(_em[_qp]) * _phi[_j][_qp] - _d_n_gamma_d_em) * _v_e_th);
		_d_denominator_d_em = (2. * _data.electrode_area() * _e[_qp] * (((-1. + 2. * _a) * _d_muem_d_actual_mean_en[_qp] * _actual_mean_en * -_phi[_j][_qp] / _voltage_scaling * _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma)) + ((-1. + 2. * _a) * (_muem[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * (std::exp(_em[_qp]) * _phi[_j][_qp] - _d_n_gamma_d_em))) * _data.ballast_resist() * (-1. + _r));
	...
	else if (jvar == _mean_en_id)
	{
		...
		_v_e_th = std::sqrt(8 * _e[_qp] * 2.0 / 3 * std::exp(_mean_en[_qp] - _em[_qp]) / (M_PI * _massem[_qp]));
		_d_v_e_th_d_mean_en = 0.5 / _v_e_th * 8 * _e[_qp] * 2.0 / 3 * _actual_mean_en / (M_PI * _massem[_qp]) * _phi[_j][_qp];
		_v_i_th = std::sqrt(8 * _kb[_qp] * _T_heavy[_qp] / (M_PI * _mass[_qp]));

		_numerator = (-2. * (1. + _r) * _u[_qp] - 2. * (1. + _r) * _V_bat.value(_t, _q_point[_qp]) + _data.electrode_area() * _e[_qp] * _data.ballast_resist() / _voltage_scaling * (-1. + _r) * ((-1. + (-1. + _a) * _se_coeff[_qp]) * _N_A[_qp] * std::exp(_ip[_qp]) * _v_i_th + _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) * _v_e_th));
		_denominator = (2. * _data.electrode_area() * _e[_qp] * ((-1. + 2. * _a) * (_muem[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) - (-1. + 2. * _b) * (-1. + (-1. + _a) * _se_coeff[_qp]) * (_muip[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * std::exp(_ip[_qp])) * _data.ballast_resist() * (-1. + _r));

		_d_numerator_d_mean_en = _data.electrode_area() * _e[_qp] * _data.ballast_resist() / _voltage_scaling * (-1. + _r) * (_N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) * _d_v_e_th_d_mean_en + _N_A[_qp] * (-_d_n_gamma_d_mean_en) * _v_e_th);
		_d_denominator_d_mean_en = (2. * _data.electrode_area() * _e[_qp] * (((-1. + 2. * _a) * _d_muem_d_actual_mean_en[_qp] * _actual_mean_en * _phi[_j][_qp] / _voltage_scaling * _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma)) + ((-1. + 2. * _a) * (_muem[_qp] * ( _t_units / ( _r_units * _r_units * _voltage_scaling ) ) ) * _N_A[_qp] * (-_d_n_gamma_d_mean_en))) * _data.ballast_resist() * (-1. + _r));
