...
NeumannCircuitVoltageMoles_KV::computeQpJacobian()
{
  ...
  _denominator = (2. * _data.electrode_area() * _e[_qp] * ((-1. + 2. * _a) * _muem[_qp] / _voltage_scaling * _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma) - (-1. + 2. * _b) * (-1. + (-1. + _a) * _se_coeff[_qp]) * _muip[_qp] / _voltage_scaling * _N_A[_qp] * std::exp(_ip[_qp])) * _data.ballast_resist() * (-1. + _r));
...
NeumannCircuitVoltageMoles_KV::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _ip_id)
  {
    ...
    _d_denominator_d_ip = (2. * _data.electrode_area() * _e[_qp] * ((-1. + 2. * _a) * _muem[_qp] / _voltage_scaling * _N_A[_qp] * (-_d_n_gamma_d_ip) - (-1. + 2. * _b) * (-1. + (-1. + _a) * _se_coeff[_qp]) * _muip[_qp] / _voltage_scaling * _N_A[_qp] * std::exp(_ip[_qp]) * _phi[_j][_qp]) * _data.ballast_resist() * (-1. + _r));
  ...
  else if (jvar == _em_id)
  {
    ...
    _d_denominator_d_em = (2. * _data.electrode_area() * _e[_qp] * (((-1. + 2. * _a) * _d_muem_d_actual_mean_en[_qp] * _actual_mean_en * -_phi[_j][_qp] / _voltage_scaling * _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma)) + ((-1. + 2. * _a) * _muem[_qp] / _voltage_scaling * _N_A[_qp] * (std::exp(_em[_qp]) * _phi[_j][_qp] - _d_n_gamma_d_em))) * _data.ballast_resist() * (-1. + _r));
  ...
  else if (jvar == _mean_en_id)
  {
    ...
    _d_denominator_d_mean_en = (2. * _data.electrode_area() * _e[_qp] * (((-1. + 2. * _a) * _d_muem_d_actual_mean_en[_qp] * _actual_mean_en * _phi[_j][_qp] / _voltage_scaling * _N_A[_qp] * (std::exp(_em[_qp]) - _n_gamma)) + ((-1. + 2. * _a) * _muem[_qp] / _voltage_scaling * _N_A[_qp] * (-_d_n_gamma_d_mean_en))) * _data.ballast_resist() * (-1. + _r));
