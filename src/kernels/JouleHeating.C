...
{
  return _test[_i][_qp] * -_grad_potential[_qp] * _voltage_scaling *
         (-_muem[_qp] * -_grad_potential[_qp] * std::exp(_em[_qp]) -
...
JouleHeating::computeQpJacobian()
{
  ...
  return _test[_i][_qp] * -_grad_potential[_qp] * _voltage_scaling *
         (-d_muem_d_mean_en * -_grad_potential[_qp] * std::exp(_em[_qp]) -
          d_diffem_d_mean_en * std::exp(_em[_qp]) * _grad_em[_qp]);
}
...
JouleHeating::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  {
    return _test[_i][_qp] * -_grad_potential[_qp] * _voltage_scaling *
               (-_muem[_qp] * -_grad_phi[_j][_qp] * std::exp(_em[_qp])) +
           _test[_i][_qp] * -_grad_phi[_j][_qp] * _voltage_scaling *
               (-_muem[_qp] * -_grad_potential[_qp] * std::exp(_em[_qp]) -
                _diffem[_qp] * std::exp(_em[_qp]) * _grad_em[_qp]);
  }
  else if (jvar == _em_id)
  {
    ...
    return _test[_i][_qp] * -_grad_potential[_qp] * _voltage_scaling *
           (-_muem[_qp] * -_grad_potential[_qp] * std::exp(_em[_qp]) * _phi[_j][_qp] -
            d_muem_d_em * -_grad_potential[_qp] * std::exp(_em[_qp]) -
            _diffem[_qp] * (std::exp(_em[_qp]) * _phi[_j][_qp] * _grad_em[_qp] +
                            std::exp(_em[_qp]) * _grad_phi[_j][_qp]) -
            d_diffem_d_em * std::exp(_em[_qp]) * _grad_em[_qp]);
  }
  else
  {
    return 0.0;
