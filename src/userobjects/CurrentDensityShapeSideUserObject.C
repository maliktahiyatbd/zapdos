...
CurrentDensityShapeSideUserObject::execute()
{
  for (unsigned int qp = 0; qp < _qrule->n_points(); ++qp)
  {
    RealVectorValue ion_current =
        _e[qp] * (_t_units / _r_units) * (_muip[qp] * -_grad_potential[qp] * std::exp(_ip[qp]) -
                                          _diffip[qp] * std::exp(_ip[qp]) * _grad_ip[qp]);
    RealVectorValue electron_current =
        -_e[qp] * (_t_units / _r_units) * (-_muem[qp] * -_grad_potential[qp] * std::exp(_em[qp]) -
  ...
  if (jvar == _ip_id)
  {
    ...
    for (unsigned int qp = 0; qp < _qrule->n_points(); ++qp)
    {
      RealVectorValue d_ion_current_d_ip =
          _e[qp] * (_t_units / _r_units) *
          (_muip[qp] * -_grad_potential[qp] * std::exp(_ip[qp]) * _phi[_j][qp] -
  ...
  else if (jvar == _em_id)
  {
    ...
    for (unsigned int qp = 0; qp < _qrule->n_points(); ++qp)
    {
      ...
      RealVectorValue d_electron_current_d_em =
          -_e[qp] * (_t_units / _r_units) *
          (-_muem[qp] * -_grad_potential[qp] * std::exp(_em[qp]) * _phi[_j][qp] -
  ...
  else if (jvar == _potential_id)
  {
    ...
    for (unsigned int qp = 0; qp < _qrule->n_points(); ++qp)
    {
      RealVectorValue d_ion_current_d_potential =
          _e[qp] * (_t_units / _r_units) * (_muip[qp] * -_grad_phi[_j][qp] * std::exp(_ip[qp]));
      RealVectorValue d_electron_current_d_potential =
          -_e[qp] * (_t_units / _r_units) * (-_muem[qp] * -_grad_phi[_j][qp] * std::exp(_em[qp]));
  ...
  else if (jvar == _mean_en_id)
  {
    ...
    for (unsigned int qp = 0; qp < _qrule->n_points(); ++qp)
    {
      ...
      RealVectorValue d_electron_current_d_mean_en =
          -_e[qp] * (_t_units / _r_units) *
          (-d_muem_d_mean_en * -_grad_potential[qp] * std::exp(_em[qp]) -
