...
{
  return -_test[_i][_qp] * _ionization_coeff * (.0382 + 2.9e5 / 760.0) / (1.0e4) * 1.0e4 *
         _grad_potential[_qp].size() * _u[_qp];
}
...
{
  return -_test[_i][_qp] * _ionization_coeff * (.0382 + 2.9e5 / 760.0) / (1.0e4) * 1.0e4 *
         _grad_potential[_qp].size() * _phi[_j][_qp];
}
...
IonizationSource::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  {
    return -_test[_i][_qp] * _ionization_coeff * (.0382 + 2.9e5 / 760.0) / (1.0e4) * 1.0e4 *
           _grad_potential[_qp] / _grad_potential[_qp].size() * _grad_phi[_j][_qp] * _u[_qp];
  }
