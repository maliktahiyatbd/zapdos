...
validParams<CoupledBEKinetic>()
{
  ...
  params.addRequiredParam<std::vector<Real>>(
      "reaction_coeffs", "The reaction coefficients multiplying the reaction source terms");
...
CoupledBEKinetic::CoupledBEKinetic(const InputParameters & parameters)
  : Kernel(parameters), _reaction_coeffs(getParam<std::vector<Real>>("reaction_coeffs"))
{
...
Real
CoupledBEKinetic::computeQpJacobian()
{
  return 0.0;
}
