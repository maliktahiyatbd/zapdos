...
template <>
InputParameters
validParams<ElectronEnergyDiffusion>()
{
  InputParameters params = validParams<Diffusion>();

  params.addRequiredCoupledVar(
      "em", "The electron density used to calculate the diffusivity using the Einstein relation");
...
ElectronEnergyDiffusion::computeQpJacobian()
{
  ...
  _D_Te = _mu_Te * _T_e;
  _d_D_Te_d_Te = _mu_Te;
  return 1.5 * _em[_qp] * _grad_test[_i][_qp] *
         (_grad_u[_qp] * _d_D_Te_d_Te * _phi[_j][_qp] + _D_Te * _grad_phi[_j][_qp]);
}

Real
ElectronEnergyDiffusion::computeQpOffDiagJacobian(unsigned int jvar)
{
  _mu_Te = 5.0 / 3.0 * _muem[_qp];
  _T_e = _std::max(_u[_qp], 0.0);
  _D_Te = _mu_Te * _T_e;
